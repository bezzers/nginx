...
curl -k --location-trusted -H "X-Vault-Token: $VAULT_TOKEN" -X GET https://$VAULT_SECRET_URL/nginx | jq -r '.data.key' | sed 's/"//g' > /etc/nginx/cert.key
curl -k --location-trusted -H "X-Vault-Token: $VAULT_TOKEN" -X GET https://$VAULT_SECRET_URL/nginx | jq -r '.data.cert' | sed 's/"//g' > /etc/nginx/cert.crt
